...
class Schedule(object):
    def __init__(self, Name, XML_url):
        ...
        if schema.validate(kebiao):  # 验证xml格式是否正确
            if Name == STUDENT_NAME:
                ...
                WORKDAY = int(
                    kebiao.xpath("HEAD/NUM_DAY/NUM_DAY_WORKDAY/text()")[0]
                )
                WEEKEND = int(
                    kebiao.xpath("HEAD/NUM_DAY/NUM_DAY_WEEKEND/text()")[0]
                )
                MORNCLASS = int(
                    kebiao.xpath("HEAD/NUM_CLASS/NUM_CLASS_MORN/text()")[0]
                )
                NOONCLASS = int(
                    kebiao.xpath("HEAD/NUM_CLASS/NUM_CLASS_NOON/text()")[0]
                )
                DUSKCLASS = int(
                    kebiao.xpath("HEAD/NUM_CLASS/NUM_CLASS_DUSK/text()")[0]
                )
                STUDENT_NAME = kebiao.xpath(
                    "HEAD/STUDENT/STUDENT_NAME/text()"
                )[0]
                STUDENT_ID = int(
                    kebiao.xpath("HEAD/STUDENT/STUDENT_ID/text()")[0]
                )
    ...
    def lesson(self, num, element):
        ...
        WHATDAY = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//WHATDAY/text()" % (the_Lesson)
        )[0]
        WEEKLIST = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//WEEK_LIST/text()" % (the_Lesson)
        )[0]
        CLASSLIST = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//CLASS_LIST/text()" % (the_Lesson)
        )[0]
        LOCATION = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//LOCATION/text()" % (the_Lesson)
        )[0]
        TEACHER = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//TEACHER/text()" % (the_Lesson)
        )[0]
        TYPE = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//TYPE/text()" % (the_Lesson)
        )[0]
        OTHER = self.kebiao.xpath(
            "BODY/LESSON[NAME='%s']//OTHER/text()" % (the_Lesson)
        )[0]
        if element == "WHATADAY":
            return WHATDAY
